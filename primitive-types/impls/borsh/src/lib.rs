...
macro_rules! impl_uint_borsh {
	($name: ident, $len: expr) => {
		...
		impl $crate::borsh::BorshDeserialize for $name {
			#[inline]
			fn deserialize(buf: &mut &[u8]) -> $crate::borsh::io::Result<Self> {
				if buf.len() < ($len * 8) {
					return Err($crate::borsh::io::Error::new(
						$crate::borsh::io::ErrorKind::InvalidInput,
		...
		// 		definitions: &mut $crate::borsh::collections::HashMap<
