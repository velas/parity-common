...
macro_rules! impl_uint_borsh {
	($name: ident, $len: expr) => {
		...
		impl $crate::borsh::BorshSchema for $name {
            fn add_definitions_recursively(definitions: &mut std::collections::HashMap<$crate::borsh::schema::Declaration, $crate::borsh::schema::Definition>) {
				let definition = $crate::borsh::schema::Definition::Array {
					length: $len,
